...
public final class StoneTablet {
    ...
    public static void openTranslator(Game game, Book bookItem) {
        if (bookItem.getName().contains("stone tablet") && (!(bookItem.getName().contains("translated") || bookItem.getName().contains("translation")) ) ) {
    ...
    private static void bookReader(Game game, Book bookItem, String input) {
        if (input.equals("exit") || input.equals("stop") || input.equals("close") || input.equals("put away")) {
            ...
            if (input.equals("init") || input.equals("help")) {
                ...
                System.out.println(" to translate the tablet successfully, the first" + Constants.STONETABLET_REQ_CORRECT
                        + " letters");
                ...
                if (inputArray.length < Constants.STONETABLET_REQ_CORRECT && inputArray.length < allTextArray.length) {
	                ...
	                if (successful) {
	                    ...
	                    game.player.addItem(new Book("translation of " + bookItem.getName(), "", ItemState.NOT_USABLE, 0, newpages));
	                    ...
	                    for (Room loopRaum: game.getRooms()) {
	                    	if (loopRaum.getRoomItemList() != null) {
		                    	...
		                    	{
		                    		if (loopItem.getName().contains("chiseled stone tablet") && loopItem.getName().contains("translated"))
		                    		{
		                    			translatedchiseled = true;
		                    		}
		                    		if (loopItem.getName().contains("decorated stone tablet") && loopItem.getName().contains("translated"))
		                    		{
	                    ...
	                    if (translatedchiseled && translateddecorated) {
	                    	...
	                    	System.out.println("You concluded that the treasure must be hidden by stone beneath the castle.");
	                    	System.out.println("Do not forget to 'use' your knowledge there");
	                    	game.player.addItem(new Book("treasureknowledge", "desc", de.dhbw.project.item.ItemState.NOT_USABLE, 0, 
	                    			Arrays.asList("You concluded that the treasure is hidden beneath the ruin, somehow behind stone. \n\nYou must now find the described place.")));
                ...
                System.out.println(" to translate the tablet successfully, at least " + Constants.STONETABLET_REQ_CORRECT
                        + " letters");
                System.out.println(" must be correctly translated");
                System.out.println("Enter 'close' to put " + bookItem.getName() + " away.");
            }
